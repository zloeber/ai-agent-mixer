...
class ConversationInitializer:
    ...
    def create_initial_state(self) -> ConversationState:
        ...
        state["metadata"]["max_cycles"] = self.config.conversation.max_cycles
        state["metadata"]["agents"] = list(self.config.agents.keys())
        
        logger.info(
            f"Initial state created with {len(system_messages)} system messages, "
            f"starting with agent '{self.config.conversation.starting_agent}'"
        )
    ...
    def validate_configuration(self) -> bool:
        ...
        if self.config.conversation.starting_agent not in self.config.agents:
            raise ValueError(
                f"Starting agent '{self.config.conversation.starting_agent}' "
